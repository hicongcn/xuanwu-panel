...
func findMise() string {
	return utils.FindMise()
}

func miseEnv() []string {
	return utils.MiseEnv()
}
...
func (s *MiseService) fetchLiveLanguages() ([]MiseLanguage, error) {
	// 使用 --json 获取格式化数据
	cmd := exec.Command(findMise(), "ls", "--json")
	// 继承父进程环境变量
	cmd.Env = miseEnv()
...
func (s *MiseService) Versions(plugin string) ([]string, error) {
	...
	// 只获取最新版本列表
	cmd := exec.Command(findMise(), "ls-remote", plugin)
	cmd.Env = append(cmd.Env, "MISE_NO_COLOR=1", "TERM=dumb")
	output, err := cmd.CombinedOutput()
...
func (s *MiseService) UseGlobal(plugin, version string) error {
	cmd := exec.Command(findMise(), "use", "-g", fmt.Sprintf("%s@%s", plugin, version))
	cmd.Env = miseEnv()
	output, err := cmd.CombinedOutput()
	if err != nil {
...
func (s *MiseService) UnsetGlobal(plugin, version string) error {
	...
	cmd := exec.Command(findMise(), "unuse", "-g", target)
	cmd.Env = miseEnv()
	output, err := cmd.CombinedOutput()
	if err != nil {
...
// SetEnv 设置全局环境变量（直接写入 settings.toml，避免 mise 创建 ~/.config/mise 目录）
func (s *MiseService) SetEnv(key, value string) error {
	path := miseSettingsPath()
	dir := filepath.Dir(path)
	if err := os.MkdirAll(dir, 0755); err != nil {
		return fmt.Errorf("创建配置目录失败: %v", err)
	}

	settings := miseReadSettings(path)
	settings[key] = value
	return miseWriteSettings(path, settings)
}

// UnsetEnv 取消全局环境变量（直接写入 settings.toml）
...
func miseSettingsPath() string {
	if dir := os.Getenv("MISE_CONFIG_DIR"); dir != "" {
		return filepath.Join(dir, "settings.toml")
	}
	home, _ := os.UserHomeDir()
	return filepath.Join(home, ".config", "mise", "settings.toml")
}

func miseReadSettings(path string) map[string]string {
...
func miseWriteSettings(path string, settings map[string]string) error {
