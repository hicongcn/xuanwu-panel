...
import (
	...
	"github.com/hicongcn/xuanwu-panel/internal/services/repo"
...
func Run(args []string) {
	...
	printHelp := func() {
		...
		fmt.Fprintf(os.Stderr, "  xuanwu reposync [参数]\n\n")
		fmt.Fprintf(os.Stderr, "参数详情:\n")
		fs.PrintDefaults()
		fmt.Fprintf(os.Stderr, "\n示例:\n")
		fmt.Fprintf(os.Stderr, "  xuanwu reposync --source-url https://github.com/xxx/repo.git --target-path $SCRIPTS_DIR$/repo1\n\n")
	}
	...
	if strings.Contains(cfg.TargetPath, constant.ScriptsDirPlaceholder) {
		scriptsDir := os.Getenv("XW_SCRIPTS_DIR")
		if scriptsDir != "" {
...
func preserve(baseDir string, paths string) func() {
	...
	tmpParent, err := os.MkdirTemp(baseDir, ".xuanwu_sync_preserve_*")
	if err != nil {
	...
	for _, p := range preservedList {
		...
		for _, fullPath := range matches {
			...
			// 同时要排除掉临时目录本身以及上级路径
			if err != nil || strings.HasPrefix(relPath, "..") || relPath == "." || strings.HasPrefix(relPath, ".xuanwu_sync_preserve") {
				continue
			}
