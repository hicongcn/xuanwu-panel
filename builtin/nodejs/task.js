...
function request(urlStr, method = 'GET', data = null) {
    const token = process.env.XWPKG_OPENAPI_TOKEN || process.env.OPENAPI_TOKEN || process.env.XWPKG_NOTIFY_TOKEN;
    if (!token) {
        throw new Error(`没有正确配置或缺少 XWPKG_OPENAPI_TOKEN 环境变量以使用 task 函数。请在玄武面板的任务设置中配置这些 Key。`);
    }
...
function getBaseUrl() {
    const url = process.env.XWPKG_OPENAPI_URL || process.env.OPENAPI_URL;
    if (url) {
