...
这组示例用于演示如何在 Xuanwu 中通过远程 Browserless / Chromium 服务执行浏览器自动化，并完成一次最小可用的页面截图。
...
同时建议在 Xuanwu 中为 Python 示例安装并使用 Python `3.11` 版本，因为过高版本的 Python 可能会出现依赖安装失败。

## 先安装语言依赖

运行脚本前，请先到 Xuanwu 的“语言依赖”页面安装对应的包，否则任务会因为缺少模块而失败。
...
- 验证 Xuanwu 到 Browserless 的网络连通性
- 验证远程浏览器地址和 Token 是否配置正确
- 快速测试浏览器自动化脚本是否能正常运行

## 推荐部署方式

建议配合 `browserless/chromium` 一起使用，再由 Xuanwu 中的脚本连接远程浏览器服务。
...
2. 在 Xuanwu 的“语言依赖”中安装对应包。
3. 按实际环境修改脚本中的 Browserless 地址和 Token。
4. 在 Xuanwu 中创建任务并运行对应脚本。
...
如果你使用的是 Browserless 这类远程浏览器服务，Playwright 只是作为客户端发起连接，不需要在玄武容器里再下载本地浏览器，所以通常不要执行 `playwright install`。
...
这组示例主要用于快速验证 Xuanwu 与远程浏览器服务之间的连通性，以及对应语言环境是否已经配置完成。
