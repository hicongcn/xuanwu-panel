...
# export XW_NOTIFY_TOKEN="${props.apiToken || 'YOUR_TOKEN'}"

# 2. 调用 (优先使用环境变量)
TOKEN="\${XW_NOTIFY_TOKEN:-${props.apiToken || 'YOUR_TOKEN'}}"
...
def send_notify(text, title="${testTitle.value}", channel="${testChannel.value}"):
    token = os.getenv("XW_NOTIFY_TOKEN", "${props.apiToken || 'YOUR_TOKEN'}")
 ...
 * 发送通知 (优先使用环境变量 XW_NOTIFY_TOKEN)
 * @param {string} text 消息内容
 */
const sendNotify = async (text) => {
  const token = (typeof process !== 'undefined' ? process.env.XW_NOTIFY_TOKEN : null) || "${props.apiToken || 'YOUR_TOKEN'}";
...
func sendNotify(title, text string) {
	token := os.Getenv("XW_NOTIFY_TOKEN")
	if token == "" {
...
function sendNotify($text, $title = "${testTitle.value}", $channel = "${testChannel.value}") {
    $token = getenv("XW_NOTIFY_TOKEN") ?: "${props.apiToken || 'YOUR_TOKEN'}";
          ...
          <div
            ...
            <div class="mb-4 border-b border-zinc-200 dark:border-zinc-800/50 pb-3">
              ...
              <p class="text-[12px] text-zinc-500 leading-normal">
                玄武面板内置了跨语言的脚本推送工具。通过环境层预装该库，您的脚本可以实现完全的“零配置”调用。
              </p>
            </div>
            ...
            <div class="space-y-5">
              ...
              <div class="space-y-2">
                ...
                <div class="group relative">
                  <div class="bg-zinc-900 dark:bg-black p-3 rounded-lg font-mono text-[12px] text-emerald-500 shadow-inner">
                    <span class="text-zinc-500 select-none">$ </span>xuanwu builtininstall
                  </div>
                  <Button variant="ghost" size="icon"
                    ...
                    @click="copyToClipboard('xuanwu builtininstall', 'install-cmd')">
                    <Check v-if="copiedBlock === 'install-cmd'" class="w-3.5 h-3.5 text-emerald-500" />
                    <Copy v-else class="w-3.5 h-3.5" />
                  </Button>
                </div>
                <p class="text-[10px] text-zinc-500 italic pl-1">
                  * 该命令会为 mise 管理的所有 Python 和 Node.js 版本安装 <code class="text-primary font-bold">xuanwu</code> 包。
                </p>
              </div>
                ...
                <div class="grid grid-cols-1 gap-3">
                  ...
                  <div class="space-y-1.5">
                    <div class="flex items-center justify-between px-1">
                      ...
                      <badge variant="outline" class="text-[8px] h-3.5 px-1 border-zinc-700 text-zinc-500">XUANWU-PY</badge>
                    </div>
                    <div class="bg-zinc-200/50 dark:bg-zinc-800/60 p-3 rounded-lg border border-zinc-200 dark:border-zinc-700/50 relative group shadow-sm">
                      <pre class="text-[11px] leading-snug"><span class="text-violet-500">import</span> xuanwu<br/>xuanwu.notify(<span class="text-emerald-600">"标题"</span>, <span class="text-emerald-600">"内容"</span>)</pre>
                      <Button variant="ghost" size="icon"
                        class="absolute right-2 top-2 h-6 w-6 text-zinc-400 opacity-0 group-hover:opacity-100 transition-all"
                        @click="copyToClipboard('import xuanwu\nxuanwu.notify(\'标题\', \'内容\')', 'py-builtin')">
                  ...
                  <div class="space-y-1.5">
                    <div class="flex items-center justify-between px-1">
                      ...
                      <badge variant="outline" class="text-[8px] h-3.5 px-1 border-zinc-700 text-zinc-500">XUANWU-JS</badge>
                    </div>
                    <div class="bg-zinc-200/50 dark:bg-zinc-800/60 p-3 rounded-lg border border-zinc-200 dark:border-zinc-700/50 relative group shadow-sm">
                      <pre class="text-[11px] leading-snug"><span class="text-violet-500">const</span> xuanwu = require(<span class="text-emerald-600">'xuanwu'</span>);<br/>xuanwu.notify(<span class="text-emerald-600">"标题"</span>, <span class="text-emerald-600">"内容"</span>);</pre>
                      <Button variant="ghost" size="icon"
                        class="absolute right-2 top-2 h-6 w-6 text-zinc-400 opacity-0 group-hover:opacity-100 transition-all"
                        @click="copyToClipboard('const xuanwu = require(\'xuanwu\');\nxuanwu.notify(\'标题\', \'内容\');', 'js-builtin')">
              ...
              <div class="p-3 rounded-lg bg-orange-500/5 border border-orange-500/10 space-y-1.5">
                ...
                <p class="text-[10px] text-zinc-500 leading-normal">
                  系统在执行脚本时会默认注入 <code class="text-zinc-700 dark:text-zinc-300">XWPKG_NOTIFY_TOKEN</code> & <code class="text-zinc-700 dark:text-zinc-300">XWPKG_NOTIFY_CHANNEL</code>。<strong>请确保您已在任务设置的"环境变量"或"机密"中配置了这两个同名 Key（系统会自动通过脚本环境生效）。</strong> 库会自动读取这些值，实现真正的免配置调用。
                </p>
              </div>
            </div>
          </div>
