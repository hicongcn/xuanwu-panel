...
function openSwaggerDocs() {
  window.open('https://hicongcn.github.io/xuanwu-panel/guide/api.html', '_blank')
}
  ...
  <div class="space-y-6">
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
      <div class="space-y-1.5">
        ...
        <Input v-model="form.title" placeholder="玄武面板" class="h-9" />
      </div>
      <div class="space-y-1.5">
      ...
      <div class="mt-6 p-4 bg-muted/30 rounded-lg border border-dashed border-border flex flex-col gap-3">
        ...
        <div class="flex items-start gap-3">
          ...
          <div class="space-y-1">
            ...
            <p class="text-xs text-muted-foreground leading-relaxed">
              清理任务在玄武面板后端服务启动时立即执行一次。在运行期间，系统将自动开启后台巡检计数器，每隔 1 小时进行周期性自动清理。
            </p>
          </div>
        </div>
      </div>
