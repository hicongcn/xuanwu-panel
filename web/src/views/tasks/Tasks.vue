...
import { Plus, Play, Pencil, Trash2, Search, ScrollText, GitBranch, Terminal, X, Loader2, RefreshCw, Zap, ZapOff, Copy, Tag, ChevronDown, Pin, PinOff, MoreHorizontal, CalendarClock } from 'lucide-vue-next'
...
import { generateXuanwuCommand } from '@/utils/repo-parser'
...
function openExportDialog(task: Task) {
  const cmd = generateXuanwuCommand(task)
  if (!cmd) {
...
<template>
  <div class="space-y-6">
        ...
        <div class="divide-y text-sm">
          <div v-for="(task, index) in tasks" :key="`large-${task.id}`"
            ...
            <div class="w-28 shrink-0 flex flex-col items-start justify-center gap-1 overflow-hidden">
              <span v-if="task.trigger_type === TRIGGER_TYPE.XUANWU_STARTUP" class="text-[10px] leading-tight bg-blue-500/10 text-blue-500 px-2 py-1 rounded-md">服务启动时</span>
        ...
        <div v-for="(task, index) in tasks" :key="`small-${task.id}`" class="p-3 hover:bg-muted/50 transition-colors">
          ...
          <div class="space-y-1.5 text-xs text-muted-foreground mb-3 px-1">
            <div class="flex items-center gap-3">
              ...
              <span v-if="task.trigger_type === TRIGGER_TYPE.XUANWU_STARTUP" class="text-[10px] leading-tight bg-blue-500/10 text-blue-500 px-1.5 py-0.5 rounded font-medium">服务启动时</span>
    ...
    <!-- 删除确认 (批量) -->
    <XuanwuDialog v-model:open="showBatchDeleteDialog" title="确认批量删除">
    ...
    <!-- 删除确认 (单个) -->
    <XuanwuDialog v-model:open="showDeleteDialog" title="确认删除任务">
    ...
    <!-- 导出指令弹窗 -->
    <XuanwuDialog v-model:open="showExportDialog" title="导出同步指令">
      <div class="space-y-4 py-2">
        <div class="text-xs text-muted-foreground leading-relaxed">
          您可以复制并分享此指令，在其他部署了玄武面板的系统上导入该仓库同步任务：
        </div>
        <div class="relative">
          <textarea
            readonly
