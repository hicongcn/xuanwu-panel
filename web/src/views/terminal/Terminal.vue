              ...
              <div v-for="cmd in cmds" :key="cmd.name"
                ...
                <div class="flex items-start justify-between gap-2">
                  <div class="flex flex-col min-w-0">
                    <span class="font-bold text-blue-400 text-xs">xuanwu {{ cmd.name }}</span>
                    <span class="text-gray-400 text-[11px] leading-tight mt-0.5">{{ cmd.description }}</span>
                  </div>
                  <button @click.stop.prevent="copyToClipboard(`xuanwu ${cmd.name}`)" class="opacity-0 group-hover:opacity-100 transition-opacity p-1 hover:bg-[#3c3c3c] rounded text-gray-400 hover:text-white focus:outline-none focus:ring-0 shrink-0 mt-0.5" :title="copiedCommand === `xuanwu ${cmd.name}` ? '已复制' : '复制命令'">
                    <Check v-if="copiedCommand === `xuanwu ${cmd.name}`" class="h-3 w-3 text-green-500" />
