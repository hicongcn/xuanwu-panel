...
def _get_headers():
    token = os.environ.get("XWPKG_OPENAPI_TOKEN") or os.environ.get("OPENAPI_TOKEN") or os.environ.get("XWPKG_NOTIFY_TOKEN")
    if not token:
        raise RuntimeError("没有正确配置或缺少 XWPKG_OPENAPI_TOKEN 环境变量以使用 task 函数。请在玄武面板的任务设置中配置这些 Key。")
    return {
...
def _get_base_url():
    url = os.environ.get("XWPKG_OPENAPI_URL") or os.environ.get("OPENAPI_URL")
    if url:
    ...
    notify_url = os.environ.get("XWPKG_NOTIFY_URL", "http://localhost:8052/api/v1/notify/send")
