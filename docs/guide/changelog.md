...
- **自定义 WebUI 支持 (New)**：新增前端自定义打包与热切换功能。面板系统彻底解耦前后端静态资源，用户可以在「系统设置 - 前端定制」中上传并管理自定义前端资源包，实现深度的主题替换与定制。
...
- **内建助手库 (Built-in SDK) (New)**：新增 Python 与 Node.js 的轻量级助手库 `xuanwu`。通过环境自动注入机制实现"零配置"通知投递，开发者无需在脚本中显式配置 TOKEN 或 URL。
- **环境自动初始化**：新增 `xuanwu builtininstall` 命令行工具，支持一键为 `mise` 管理的所有多语言版本同步安装/刷新内建包依赖。
...
- **日志自动脱敏**：实时流水日志自动扫描并掩码脱敏显示机密内容（`********`），防止执行时通过任务输出泄露机密。
- **严格隔离机制**：机密仅在定时调度任务时生效，终端命令执行、手动测试、调试等入口物理隔离机密。
...
- **解析器优化**：优化定时任务 Cron 规则解析逻辑，修正带 `#` 注释行的识别问题。
