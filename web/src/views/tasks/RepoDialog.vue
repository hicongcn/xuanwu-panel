...
import { api, type Task, type RepoConfig, type MiseLanguage } from '@/api'
import { toast } from 'vue-sonner'
import { cn } from '@/lib/utils'
import { getCronDescription } from '@/utils/cron'
import { parseXuanwuCommand, parseQlCommand } from '@/utils/repo-parser'
...
const showXuanwuImportDialog = ref(false)
const xuanwuCommandInput = ref('')

function importFromXuanwu() {
  xuanwuCommandInput.value = ''
  showXuanwuImportDialog.value = true
}

function submitXuanwuImport() {
  const result = parseXuanwuCommand(xuanwuCommandInput.value)
  if (!result) {
    if (!xuanwuCommandInput.value.trim()) {
      showXuanwuImportDialog.value = false
    } else {
...
<template>
        ...
        <DialogHeader class="px-5 sm:px-6 pr-20 pt-6 pb-2 shrink-0">
          <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 sm:gap-2">
            ...
            <div v-if="!isEdit" class="flex flex-wrap items-center gap-2 sm:mr-4">
              <Button variant="outline" size="sm" @click="importFromXuanwu" class="flex-1 sm:flex-initial h-8 gap-1.5 bg-primary/5 hover:bg-primary/10 border-primary/20 hover:border-primary/40 text-primary px-3">
                <Terminal class="w-3.5 h-3.5" />
                <span class="text-xs">Xuanwu 命令导入</span>
              </Button>
            ...
            <section class="space-y-4">
              ...
              <div class="grid gap-4 pl-3 border-l border-muted">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-4 items-center gap-3">
                  ...
                  <div class="sm:col-span-3">
                    <DirTreeSelect :model-value="repoConfig.target_path || ''"
  ...
  <!-- Xuanwu 导入提示对话框 -->
  <Dialog :open="showXuanwuImportDialog" @update:open="v => showXuanwuImportDialog = v">
    <DialogContent class="sm:max-w-[550px] p-0 border-none bg-background shadow-xl overflow-hidden">
      ...
      <div class="px-6 py-4 space-y-5">
        ...
        <div class="space-y-2">
          <div class="flex items-center justify-between">
            ...
            <button class="text-[10px] text-primary hover:underline font-medium" @click="xuanwuCommandInput = 'xuanwu reposync --source-url \'https://github.com/example/repo.git\' --branch \'main\' --blacklist \'test|dev\' --pre-command \'npm install\' --post-command \'echo done\''">填入示例</button>
          </div>
          <div class="p-3 rounded-lg bg-muted/40 font-mono text-[11px] text-muted-foreground/70 border border-muted/20 leading-relaxed break-all">
            xuanwu reposync --source-url 'https://...' --branch 'main' --blacklist '...' --pre-command '...' --post-command '...'
          </div>
        </div>

        <div class="relative group">
          <textarea 
            v-model="xuanwuCommandInput" 
            placeholder="在此处粘贴完整指令，如 xuanwu reposync --source-url ..." 
            ...
            @keydown.enter.ctrl.prevent="submitXuanwuImport"
          />
      ...
      <DialogFooter class="px-6 pb-6 pt-2 flex gap-2">
        <Button variant="ghost" size="sm" @click="showXuanwuImportDialog = false" class="flex-1 h-9 rounded-md font-medium text-xs">
          取消
        </Button>
        <Button size="sm" @click="submitXuanwuImport" class="flex-1 h-9 rounded-md font-bold text-xs shadow-sm bg-primary hover:bg-primary/90">
          确认解析并填充
        </Button>
      </DialogFooter>
    </DialogContent>
