...
// BuildRuntimeProcessEnv 构造 Xuanwu 内部可信子进程需要继承的运行时环境变量。
// 仅包含 Xuanwu 自己的路径/数据库配置，不包含用户任务环境变量。
func BuildRuntimeProcessEnv() []string {
	...
	envs = append(envs, formatEnvVar("XW_CONFIG_PATH", configPath))

	if scriptsDir := ResolveAbsScriptsDir(); strings.TrimSpace(scriptsDir) != "" {
		envs = append(envs, formatEnvVar("XW_SCRIPTS_DIR", scriptsDir))
	}

	appendEnvIfSet(&envs, "XW_DB_TYPE", constant.RuntimeDBType)
	appendEnvIfSet(&envs, "XW_DB_HOST", constant.RuntimeDBHost)
	if constant.RuntimeDBPort > 0 {
		envs = append(envs, formatEnvVar("XW_DB_PORT", fmt.Sprintf("%d", constant.RuntimeDBPort)))
	}
	appendEnvIfSet(&envs, "XW_DB_USER", constant.RuntimeDBUser)
	appendEnvIfSet(&envs, "XW_DB_PASSWORD", constant.RuntimeDBPassword)
	appendEnvIfSet(&envs, "XW_DB_NAME", constant.RuntimeDBName)
	appendEnvIfSet(&envs, "XW_DB_PATH", constant.RuntimeDBPath)
	appendEnvIfSet(&envs, "XW_DB_DSN", constant.RuntimeDBDSN)
	appendEnvIfSet(&envs, "XW_DB_TABLE_PREFIX", constant.RuntimeDBTablePrefix)
...
// ResolveAbsScriptsDir 解析 Xuanwu 运行时脚本目录的绝对路径。
func ResolveAbsScriptsDir() string {
	if scriptsDir := os.Getenv("XW_SCRIPTS_DIR"); scriptsDir != "" {
